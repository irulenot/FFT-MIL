...
class BRACSDataset(Dataset):
    def __init__(self, args, split='train'):
        ...
        self.bag_all = []
        if split == 'train':
            self.data_csv = pd.read_csv(join('datafolders/bracs/', 'bracs_train.csv'))
        elif split == 'test':
            self.data_csv = pd.read_csv(join('datafolders/bracs/', 'bracs_test.csv'))
        elif split =='val':
            self.data_csv = pd.read_csv(join('datafolders/bracs/', 'bracs_val.csv'))
        # Features missing
        self.data_csv = self.data_csv.reset_index(drop=True)
        ...
        if isdir(join(args.dataroot, 'single_b' + str(args.backgrd_thres))):
            ...
            func = lambda row: join(args.dataroot, row + ".pt")
        ...
        #print(self.label_list)
        if args.quick:
            self.data_csv = self.data_csv.head(16)
            self.data_csv['subject_id'] = self.data_csv['subject_id'].head(16)
            self.dir_list = self.dir_list[:16]
            self.label_list = self.label_list[:16]
        for i in range(len(self.dir_list)):
            label,feature = self.get_bag_feats(self.dir_list[i],self.label_list[i])
            self.featureList.append(feature) 
            self.bag_all.append(label)
    ...
    def get_bag_feats(self, dir,label_bag):
        ...
        patches = torch.load(feats_csv_path, weights_only=True)
        indices = torch.randperm(patches.size(0))
        shuffled_patches = patches[indices]
        feats = shuffled_patches.numpy()
...
class IMPDataset(Dataset):
    def __init__(self, args, split='train'):
        ...
        self.bag_all = []
        if split == 'train':
            self.data_csv = pd.read_csv(join('datafolders/imp/', 'imp_train.csv'))
        elif split == 'test':
            self.data_csv = pd.read_csv(join('datafolders/imp/', 'imp_test.csv'))
        elif split =='val':
            self.data_csv = pd.read_csv(join('datafolders/imp/', 'imp_val.csv'))
        # Features missing
        self.data_csv = self.data_csv.reset_index(drop=True)
...
class LUADDataset(Dataset):
    def __init__(self, args, split='train'):
        ...
        if split == 'train':
            self.data_csv = pd.read_csv(join('datafolders/luad_fft/', 'luad_train.csv'))
        elif split == 'test':
            self.data_csv = pd.read_csv(join('datafolders/luad_fft/', 'luad_test.csv'))
        elif split =='val':
            self.data_csv = pd.read_csv(join('datafolders/luad_fft/', 'luad_val.csv'))
        # Features missing
        self.data_csv = self.data_csv.reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        label = F.one_hot(label, num_classes=self.num_classes).to(float)
        feats = torch.load(self.dir_list[idx], weights_only=True)
        fft_path = '/data/fftmil/LUAD/luad_fft/' + self.dir_list[idx].split('/')[-1].split('.')[0] + '.npz'
        fft = torch.tensor(np.load(fft_path)['arr_0'])
        if self.split == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)
...
class BRACSDataset_fft(Dataset):
    def __init__(self, args, split='train'):
        super().__init__()
        self.args = args
        self.split = split
    ...
    def __getitem__(self, idx):
        ...
        label = F.one_hot(label, num_classes=self.num_classes).to(float)
        feats = torch.load(self.dir_list[idx], weights_only=True)
        fft_path = '/data/fftmil/BRACS/bracs_fft/' + self.dir_list[idx].split('/')[-1].split('.')[0] + '.npz'
        fft = torch.tensor(np.load(fft_path)['arr_0'])
        if self.split == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)

        return feats, label, fft
        
    def __len__(self):
        return len(self.data_csv)

class LUADDataset_fft(Dataset):
    def __init__(self, args, split='train'):
        super().__init__()
        self.args = args
        self.split = split
    ...
    def __getitem__(self, idx):
        ...
        label = F.one_hot(label, num_classes=self.num_classes).to(float)
        feats = torch.load(self.dir_list[idx], weights_only=True)
        fft_path = '/data/fftmil/LUAD/luad_fft0/' + self.dir_list[idx].split('/')[-1].split('.')[0] + '.npz'
        fft = torch.tensor(np.load(fft_path)['arr_0'])
        if self.split == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)
...
class IMPDataset_fft(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = F.one_hot(label, num_classes=self.num_classes).to(float)
        feats = torch.load(self.dir_list[idx], weights_only=True)
        fft_path = '/data/fftmil/IMP/imp_fft/' + self.dir_list[idx].split('/')[-1].split('.')[0] + '.npz'
        fft = torch.tensor(np.load(fft_path)['arr_0'])
        if self.split == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)
...
class CAM16Dataset_fft(Dataset):
    def __init__(self, args, split='train'):
        super().__init__()
        self.args = args
        self.split = split
        ...
        self.num_classes = args.num_classes
        if split == 'train':
            self.data_csv = pd.read_csv(join('datafolders/cam16/', 'cam16_train.csv'))
        elif split == 'test':
            self.data_csv = pd.read_csv(join('datafolders/cam16/', 'cam16_test.csv'))
        elif split =='val':
            self.data_csv = pd.read_csv(join('datafolders/cam16/', 'cam16_val.csv'))
        # Features missing
        self.data_csv = self.data_csv.reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        label = F.one_hot(label, num_classes=self.num_classes).to(float)
        feats = torch.load(self.dir_list[idx], weights_only=True)
        fft_path = '/data/fftmil/CAMELYON16/cam16_fft/' + self.dir_list[idx].split('/')[-1].split('.')[0] + '.npz'
        fft = torch.tensor(np.load(fft_path)['arr_0'])
        if self.split == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)
...
class C16DatasetV3_fft(Dataset):
    def __init__(self, args, split='train'):
        super().__init__()
        self.fft_path = args.fft_path
        self.args = args
        self.split = split
        self.featureList = []
        self.bag_all = []
        ...
        if args.quick:
            ...
            self.label_list = self.label_list[:16]
        for i in range(len(self.dir_list)):
            label,feature,fft_path = self.get_bag_feats(self.dir_list[i],self.label_list[i])
            self.featureList.append(feature) 
            self.bag_all.append(label)
    ...
    def get_bag_feats(self, dir,label_bag):
        ...
        id = feats_csv_path.split('/')[-1].split('.')[0]
        fft_path = self.fft_path + id + '.npz'

        label = torch.tensor(np.array(label))
        feats = torch.tensor(np.array(feats)).float()

        return label, feats, fft_path
    
    def __getitem__(self, idx):
        #label, feats = self.get_bag_feats(self.data_csv.iloc[idx])
        label =  self.bag_all[idx]  
        feats =  self.featureList[idx]
        fft = torch.tensor(np.load(self.fft_paths[idx])['arr_0'])
        if self.split == 'train' and self.args.dropout_patch > 0.0:
            #print('drop')
            feats = dropout_patches(feats, self.args.dropout_patch)
