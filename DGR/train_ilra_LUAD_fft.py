...
from utils import *
from wsi_dataloader_3 import C16DatasetV3,C16DatasetV4,dropout_patches, C16DatasetV3_tcga,LUADDataset_fft


from models.ILRA_fft import ILRA_fft
...
def train(trainloader, milnet, criterion, optimizer, args):
    ...
    for batch_id, (feats,label,fft) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        bag_prediction, _,_ = milnet(bag_feats, fft)
...
def test(testloader, milnet, criterion, args):
    ...
    test_predictions = []
    with torch.no_grad():
        for batch_id, (feats,label,fft) in enumerate(testloader):
            bag_feats = feats.cuda()
            bag_label = label.cuda()
            fft = fft.cuda()
            bag_feats = bag_feats.view(1, -1, args.feats_size)
...
def main():
    ...
    parser.add_argument('--dataroot', default="/data/fftmil/LUAD/features_CLAM/pt_files/", type=str, help='dataroot for the CAMELYON16 dataset')
    ...
    parser.add_argument('--save_dir', default='results/ilra_luad_fft/', type=str, help='the directory used to save all the output')
    parser.add_argument('--fold', default='0', type=str, help='fold')
    parser.add_argument('--quick', default=False, type=bool)
    parser.add_argument('--fft_path', default='/data/fftmil/LUAD/luad_fft/', type=str)
    ...
    # <------------- define MIL network ------------->
    milnet = ILRA_fft(feat_dim=args.feats_size, n_classes=args.num_classes, hidden_feat=256, num_heads=8, topk=1, ln=True).cuda()
    if args.optim == 'RAdam':
    ...
    trainset = LUADDataset_fft(args, 'train')
    testset = LUADDataset_fft(args, 'test')
    ...
    for epoch in tqdm(range(1, args.num_epochs + 1)):
        ...
        save_dir = './results/ilra_LUAD_fft_add2_lvl1/'
